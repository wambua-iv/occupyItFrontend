...
function BookingVerication({ booking }: any) {
  const [authState] = useContext(AuthContext);
  const [createAlert, setCreateAlert] = useState<boolean>(false);

  const onSubmit = async (data: any) => {
    setCreateAlert((prev: boolean) => !prev);
    const url = 'https://occupy-it.herokuapp.com/owners/accept_tenaant';
    await fetch(url, {
      method: 'PATCH',
      headers: {
        ...
        >
          ...
          >
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', mb: 1 }}>
                <Phone /> <Typography sx={{ ml: 2 }}>0712545454</Typography>
              </Box>
              <Box sx={{ display: 'flex', mb: 1 }}>
                <Email />{' '}
                <Typography sx={{ ml: 2 }}>{booking.tenant.email}</Typography>
              </Box>
            </Box>
          </Box>
          <Box
          ...
          >
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Typography>{booking.tenant.id}</Typography>
...
export default BookingVerication;
